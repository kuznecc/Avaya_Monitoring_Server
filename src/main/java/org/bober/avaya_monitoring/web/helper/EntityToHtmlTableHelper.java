...
//      Example:  <select> <option value="saab" selected>Saab</option> </select>
    ...
    public String checkConfigToHtmlTableEditableRow(CheckConfig checkConfig,
        ...
        final String btnCancelLinkUrl = "/configuration/getCheckConfigRow/" +
                checkConfig.getDbTableName() + "/" + checkConfig.getId();
        String btnCancelPattern =
                "<button onClick=\"replaceTagWithHtml('"+rowId+"', '" + btnCancelLinkUrl + "')\">cancel</button>";

        String btnSavePattern =
                "<button onClick=\"replaceTagWithHtml('"+rowId+"', '" + btnCancelLinkUrl + "')\">save</button>";
        ...
        String editableCellPattern = "<td><input id=\"%s\" type=\"text\" size=\"%d\" value=\"%s\"></td>";

        result.append(String.format(simpleCellPattern , checkConfig.getId() ));

        result.append(String.format(simpleCellPattern ,
                getHtmlDropDownList("cfgEntityIdDD", listOfAllMonitoredEntities,checkConfig.getEntityId())
        ));

        result.append(String.format(editableCellPattern , "cfgAttributes", 10,
                (checkConfig.getAttributes()==null)?"":checkConfig.getAttributes()
        ));

        result.append(String.format(editableCellPattern , "cfgFrequency", 10,
                checkConfig.getFrequency()
        ));

        result.append(String.format(simpleCellPattern,
                        getHtmlCheckBox("isDisabled"+rowId, checkConfig.isDisabled()))
        );

        result.append(String.format(editableCellPattern , "cfgDescription", 45,
                (checkConfig.getDescription()==null)?"":checkConfig.getDescription()
        ));

        result.append(String.format(simpleCellPattern, btnSavePattern + btnCancelPattern ) );
    ...
    public String checkConfigToHtmlTableRow(CheckConfig checkConfig) {
        ...
        String btnEditPattern =
                "<button onClick=\"replaceTagWithHtml('"+rowId+"', '" + btnEditLinkUrl + "')\">edit</button>";
